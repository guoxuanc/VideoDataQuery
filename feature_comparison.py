...
def display_plot(final_scores, title, save_location=None):
    ...
    for i, score in enumerate(final_scores):
        ...
        if i == 0:
            ...
            plt.plot(np.argmax(Y), score[1], marker='d', markersize=7, label=score[0], color='goldenrod')
            plt.annotate("{}\n({}s)".format(score[0], round(np.argmax(Y)/config.FRAME_RATE)), (np.argmax(Y), score[1]), textcoords="offset points", xytext=(0,15), ha='center', size='small', color='goldenrod', fontweight='heavy')
        elif np.absolute(prev_fr - np.argmax(Y)) > 50:
            plt.plot(np.argmax(Y), score[1], marker='d', markersize=7, label=score[0], color='darkslategray')
            plt.annotate("{}\n({}s)".format(score[0], round(np.argmax(Y)/config.FRAME_RATE)), (np.argmax(Y), score[1]), textcoords="offset points", xytext=(0,-17), ha='center', size='xx-small', color='darkslategray', fontweight='bold')
        else:
            plt.plot(np.argmax(Y), score[1], marker='d', markersize=7, label=score[0], color='black')
            plt.annotate("{}\n({}s)".format(score[0], round(np.argmax(Y)/config.FRAME_RATE)), (np.argmax(Y), score[1]), textcoords="offset points", xytext=(0,-10*((-1)**i)), ha='center', size='xx-small', color='black', fontweight='bold')
        prev_fr = np.argmax(score[3]) 
...
def generate_plots(final_scores, title, save_location=None):
    ...
    f.suptitle(title)
    lg = plt.legend(labels, ncol=2)
    lg.get_texts()[0].set_text('color_luma')
    lg.get_texts()[1].set_text('color_red')
    lg.get_texts()[2].set_text('color_green')
    lg.get_texts()[3].set_text('color_blue')
    lg.get_texts()[4].set_text('audio_spectral')
    lg.get_texts()[5].set_text('hash_average')
    lg.get_texts()[6].set_text('hash_persepctive')
    lg.get_texts()[7].set_text('hash_wavelet')
    lg.get_texts()[8].set_text('hash_differ_hor')
    lg.get_texts()[9].set_text('hash_differ-ver')
    lg.get_texts()[10].set_text('motion_vectors_x')
    lg.get_texts()[11].set_text('motion_vectors_y')
